...
define LINUX_KCONFIG_FIXUP_CMDS
	...
	$(if $(BR2_PACKAGE_LINUX_TOOLS_PERF),
		...
		$(call KCONFIG_ENABLE_OPT,CONFIG_FUNCTION_TRACER,$(@D)/.config)
		$(call KCONFIG_ENABLE_OPT,CONFIG_DYNAMIC_FTRACE,$(@D)/.config)
	))
	$(if $(BR2_PACKAGE_PCM_TOOLS),
		$(call KCONFIG_ENABLE_OPT,CONFIG_X86_MSR,$(@D)/.config))
	$(call KCONFIG_ENABLE_OPT,CONFIG_KGDB,$(@D)/.config)
	$(call KCONFIG_ENABLE_OPT,CONFIG_KGDB_SERIAL_CONSOLE,$(@D)/.config)
	$(call KCONFIG_ENABLE_OPT,CONFIG_KGDB_KDB,$(@D)/.config)
	$(call KCONFIG_ENABLE_OPT,CONFIG_DEBUG_INFO,$(@D)/.config)
	$(call KCONFIG_DISABLE_OPT,CONFIG_IMX2_WDT,$(@D)/.config)
